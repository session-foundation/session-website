            ...
            return (
              <li key={locale}>
                <Link
                  href={router.asPath}
                  locale={locale}
                  className={classNames('block w-full rounded-md px-4 py-3 text-left transition-colors duration-200 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2', isCurrentLocale
                    ? 'bg-green-50 font-semibold text-primary-dark'
                    : 'text-gray-700 hover:text-gray-900')}
                  aria-current={isCurrentLocale ? 'page' : undefined}
                  aria-label={t(isCurrentLocale ? 'aria.currentLanguage': 'aria.switchToLanguage', { language: languageName })}
                >
